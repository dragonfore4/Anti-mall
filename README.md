...
`CHECKPOINTS` คือ array ของจุดมรดก ๑๐ จุด แต่ละจุดมี `coord: [lat,lng]`, `points` (แต้ม), `fact` (เกร็ดความรู้)
...
- **`basic(spec)`** — factory: รับแค่ `checkpointIds` แล้ว **เติม `path` + `distanceKm` + `legCalories` ให้อัตโนมัติ** เพิ่มเส้นทางใหม่แค่ก๊อป block แล้วใส่ id หมุด ไม่ต้องพิมพ์พิกัดเอง
- **`legCalories: number[]`** = แคลอรี่ **ต่อช่วง** (leg) ยาว = จำนวนหมุด − 1 เช่น `[90, 70, 130]` คือช่วง ๑→๒ ได้ 90, ๒→๓ ได้ 70, ๓→๔ ได้ 130 · กรอกเองในแต่ละ route หรือเว้นไว้ให้คิดจากระยะ (`legCaloriesFromPath`)
...
4. **เช็คอิน** — วน checkpointIds ถ้าเข้าใกล้หมุดที่ index `i` < 45ม. (`CHECKIN_RADIUS_M`) และยังไม่เคยเช็คอิน → บวกแต้ม + บวกแคลของช่วงที่เพิ่งวิ่งมา (`route.legCalories[i-1]`, จุดเริ่ม = 0) + เด้ง toast
...
- `stats.ts`: `steps(distanceM)` (ประมาณก้าว = ระยะ ÷ 0.75ม.), `formatTime`, ค่าคงที่ `CHECKIN_RADIUS_M` / `GPS_JUMP_MAX_M`
> หมายเหตุ: **แคลอรี่ไม่ใช้สูตร MET แล้ว** — เป็นค่า **ต่อช่วง** กำหนดใน `legCalories` ของแต่ละเส้นทาง (routes.ts) บวกสะสมตอนเช็คอินใน store
...
| เปลี่ยนแคลอรี่ต่อช่วง (leg) | [`data/routes.ts`](src/data/routes.ts) — แก้ array `legCalories` ของเส้นทางนั้น |
| เพิ่มเส้นทาง Basic | [`data/routes.ts`](src/data/routes.ts) — ก๊อป `basic({...})` ใส่ id หมุด + `legCalories` (หรือเว้นให้คิดจากระยะ) |
